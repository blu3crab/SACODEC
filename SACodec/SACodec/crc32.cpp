...
DWORD genCRC32( HANDLE file )
{
	...
	{
		if( ReadFile( file, buffer, MAX_READ_WRITE_SIZE, &bytesRead, NULL ) )
		{
			for( int i = 0; i < ( signed )bytesRead; i++ )
				crc = ( crc >> 8 ) ^ crc32Table[ buffer[ i ] ^ ( crc & 0xFF ) ];
		}
	}

	//reflection required by crc stardard
	return ~crc;
}